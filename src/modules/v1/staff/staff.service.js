...
export const listStaff = async (queryString) => {
  ...
  const features = new APIFeatures(Staff.find().populate('user department'), queryString)
    .filter(['employeeCode'])
    .sort('-createdAt')
...
export const softDeleteStaff = async (id, req) =>
  withTransaction(async (session) => {
    ...
    await Attendance.updateMany({ staff: id }, { isDeleted: true, deletedAt: new Date() }).session(session);
    await Deduction.updateMany({ staff: id }, { isDeleted: true, deletedAt: new Date() }).session(session);
...
export const getAttendance = async (staffId, queryString) => {
  ...
  const features = new APIFeatures(Attendance.find({ staff: staffId }), queryString)
    .sort('-date')
    .paginate(totalDocuments);
  return { results: await features.query, pagination: features.pagination };
};
...
export const getMonthlyAttendanceSummary = async (staffId, month) => {
  ...
  return {
    ...
    hoursWorked: records.reduce((sum, record) => sum + (record.workingHours || 0), 0)
  };
};
...
export const listDeductions = async (staffId, queryString) => {
  ...
  const features = new APIFeatures(Deduction.find({ staff: staffId }), queryString)
    .sort('-createdAt')
    .paginate(totalDocuments);
  return { results: await features.query, pagination: features.pagination };
};
...
export const updateDeduction = async (staffId, deductionId, payload, req) => {
  const before = await Deduction.findOne({ _id: deductionId, staff: staffId }).lean();
  const deduction = await Deduction.findOneAndUpdate({ _id: deductionId, staff: staffId }, payload, {
    new: true,
    runValidators: true
  });
...
export const calculateSalary = (staffId, month) => calculateMonthlySalary(staffId, month);
...
export const paySalary = async (staffId, month, req) =>
  withTransaction(async (session) => {
    ...
    if (existing && existing.isPaid) throw new AppError('Salary already paid for this month', 400);

    if (existing) {
      Object.assign(existing, { ...result, month, isPaid: true, paidAt: new Date() });
    } else {
      staff.monthlyReports.push({ ...result, month, isPaid: true, paidAt: new Date() });
    }
...
export const adjustSalary = async (staffId, month, adjustments, req) => {
  ...
  const report = staff.monthlyReports.find((item) => item.month === month);
  if (report) {
    report.adjustments = adjustments;
  } else {
    staff.monthlyReports.push({ month, adjustments });
  }
  await staff.save();
